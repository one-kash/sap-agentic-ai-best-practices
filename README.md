...
Progress tracking should capture:
- Overall statistics (total, processed, successful, failed)
- Per-object status (pending, in_progress, completed, failed)
- Output file references for completed objects
...
On resume, perform validation checks:
1. Verify progress file structure integrity
2. Confirm object counts match discovery
3. Check that output files exist for "completed" objects
4. Reconcile any inconsistencies (reset missing files to pending)
...
- Default batch size of 3-5 objects
- Checkpoint progress after each batch
- Continue processing if individual objects fail
- Track failures for post-processing review

Tune batch size based on:
- Network latency (smaller batches for high latency)
...
1. **Discovery**: Search for Z*/Y* objects in target package
2. **ATC Analysis**: Run ATC checks with Clean Core variant
3. **Classification**: Assign Clean Core level (A/B/C/D) based on findings
4. **Reporting**: Generate summary with level distribution

| Level | Classification | Meaning |
|-------|---------------|---------|
| A | Fully Clean | Uses only released APIs |
| B | Pragmatically Clean | Minor non-critical findings |
| C | Conditionally Clean | Requires migration planning |
